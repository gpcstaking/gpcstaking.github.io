...
function formatRewardMultiplier(staticReward: bigint, communityReward: bigint, language: Language) {
  if (staticReward === 0n) return "1.00";
  const multiplierBps = (staticReward + communityReward) * BPS / staticReward;
  return new Intl.NumberFormat(language === "zh" ? "zh-CN" : "en-US", {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,
  }).format(Number(multiplierBps) / Number(BPS));
}
    ...
    <main className="mobile-stage">
      <div className="dapp-shell" id="top">
            ...
            {!account ? (
              ...
              <>
                ...
                <article className="service-action-card withdraw-card">
                  <div className="service-action-title"><span className="heading-icon"><DappIcon name="withdraw" size={17} /></span><div><strong>{text("代提现", "Assisted claim")}</strong></div></div>
        ...
        <div className="tab-page" hidden={serviceMode || activeTab !== "team"}>
          ...
          <div className="reward-note"><DappIcon name="shield" size={16} /><div><strong>{text("社区收益按有效算力计算", "Community rewards use effective power")}</strong><span>{text("小区有效算力最高为个人算力的 5 倍，收益最高加速至 1.5 倍；无论是否覆盖小区总算力，均按实际有效算力日收益的 10% 计算社区奖励。", "Effective small-area power is capped at 5× personal power, with a maximum 1.5× reward boost. Community rewards always equal 10% of the daily return generated by the actual effective power, even when it is below total small-area power.")}</span></div></div>
        </div>
