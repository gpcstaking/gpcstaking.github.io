...
The mobile wallet interface is under `frontend/`. Its audited BSC proxy address is pinned in source and CI rejects a mismatched deployment variable. The interface supports BSC wallet switching with PancakeSwap MEV Guard as the add-network RPC, mandatory first-entry referral binding, exact test-stage 1 USDT approval (including reducing legacy oversized allowances), pre-signing Pancake quotes with a 0.3% user floor and 60-second deadline, GPC staking, reward quotes, community statistics, paginated direct-referral reads, and withdrawals. A non-navigable assisted-operations view lets any wallet pay for an order credited to a bound beneficiary. Only the operation wallet may trigger a beneficiary's available withdrawal; that authorization is enforced by the proxy implementation, and withdrawal proceeds always go to the beneficiary. Personal-power and promotion-quota details are read directly from the separate on-chain history registry, which keeps the latest 30 packed records for each ledger and avoids archive-event RPC scans.
